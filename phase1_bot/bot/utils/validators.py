...
# TRC20 (Tron) address pattern — used for USDT/TRC20
...
def detect_currency_from_address(address: str) -> Optional[str]:
    """Auto-detect cryptocurrency type from wallet address format.

    Returns: "BTC", "ETH", "LTC", "USDT", or None if unrecognised.
    """
    if not address:
        return None
    address = address.strip()
    if AddressValidator.BTC_PATTERN.match(address):
        return "BTC"
    if AddressValidator.ETH_PATTERN.match(address):
        return "ETH"
    if AddressValidator.LTC_PATTERN.match(address):
        return "LTC"
...
class TxHashValidator:
    ...
    def validate_tx_hash(tx_hash: str, currency: str) -> Tuple[bool, str]:
        ...
        return True, "✅ Transaction hash recorded"
